...
- **Some OpenRouter slugs don't have open weights**, despite appearing under a vendor we treat as open. We filtered these out: all Qwen cloud tiers (Max/Plus/Turbo/Coder-Plus/Coder-Flash/3.5-Flash/3.5-Plus/3.6-Plus), Mistral's paid-tier models (Large/Medium/Ministral-Premium/Pixtral-Large/Voxtral/Saba), xAI Grok cloud-only variants, all Google Gemini, all Anthropic, all OpenAI except `gpt-oss-*`, all Stepfun/MiMo/Arcee-cloud. If the OpenRouter slug is missing from this file and you think it belongs, verify the model card on HF explicitly publishes weights under an open license.
