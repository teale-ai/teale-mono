...
to the Teale network. Bundled `libllamaserver.so` + `libtealenode.so` launch when
the toggle is on, but the 769-MB GGUF model has to be pushed once over USB:
...
Then flip Settings → Supply inference. The phone registers with the relay, serves
incoming requests, and earns `DIRECT_EARN` + `AVAILABILITY_DRIP` entries visible
on the Wallet tab.
