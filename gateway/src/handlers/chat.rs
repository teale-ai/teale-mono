...
pub async fn chat_completions(
    ...
    if streaming {
        let stream =
            run_streaming(state, catalog_model, req, consumer_device_id, required_ctx).await?;
        Ok(stream.into_response())
    } else {
        let json =
            run_buffered(state, catalog_model, req, consumer_device_id, required_ctx).await?;
        Ok(json.into_response())
    }
}

/// Rough estimate of the context window a request will consume. Used only
/// for `teale/auto` resolution — NOT for hard request validation (the
/// gateway still trusts nodes to honor their advertised n_ctx).
///
